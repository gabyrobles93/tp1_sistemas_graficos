class CircleIncomplete {
    constructor(vertices, radius, proportion) {
    ...
    getPosition(u, level) {
        var x = this.radius * Math.cos(this.proportion*2*Math.PI * u);
        var y = this.radius * Math.sin(this.proportion*2*Math.PI * u);
        return [x, y, 0];
    }
    ...
    getTangent(u) {
        var x = -1 * this.radius * 2 * Math.PI * Math.sin(this.proportion*2*Math.PI * u);
        var y = this.radius * 2 * Math.PI * Math.cos(this.proportion*2*Math.PI * u);
        return [x, y, 0];
    }
